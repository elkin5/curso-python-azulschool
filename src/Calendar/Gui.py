...
### imports ###
import sys, src.Calendar.Events as ce, tkinter as tk, datetime as dt
...
class EventoForm:
    ...
    def __init__(self, root, evento=None, *args, **kwargs):
        ...
        self.btn_add_asist = ttk.Button(self.window, text="Agregar",
                                        command=lambda: AsistenteForm(listbox=self.asistentes, root=self.window))
        self.btn_add_asist.grid(row=1, column=5, sticky="ew", pady=5, padx=5)

        self.btn_del_ass = ttk.Button(self.window, text="Eliminar",
                                      command=lambda: self.asistentes.delete(self.asistentes.curselection()))
        self.btn_del_ass.grid(row=2, column=5, sticky="ew", pady=5, padx=5)

        self.btn_clean_desc = ttk.Button(self.window, text="Limpiar",
                                         command=lambda: self.descripcion.delete(1.0, tk.END))
